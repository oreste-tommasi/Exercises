...
{
	m_OpenGLPage = ref new OpenGLPage();

	if (args->PreviousExecutionState == ApplicationExecutionState::Terminated)
	{
		m_OpenGLPage->LoadInternalState(ApplicationData::Current->LocalSettings->Values);
	}

	// Place the page in the current window and ensure that it is active.
	Window::Current->Content = m_OpenGLPage;
...
void App::OnSuspending(Object^ sender, SuspendingEventArgs^ args)
{
	...
	m_OpenGLPage->SaveInternalState(ApplicationData::Current->LocalSettings->Values);
}